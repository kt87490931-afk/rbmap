...
interface WidgetsAData {
  price_rows?: { region?: string; type?: string; val?: string; chg?: string }[];
  ...
  keywords?: { href?: string; rank?: string; text?: string; hot?: boolean }[];
}

const DEFAULT: WidgetsAData = {
  price_rows: [
...
export default function WidgetRowA({ data }: { data?: WidgetsAData | null }) {
  ...
  const priceRows = d.price_rows ?? DEFAULT.price_rows ?? [];
  const venueRanks = d.venue_ranks ?? DEFAULT.venue_ranks ?? [];
  const categories = d.categories ?? DEFAULT.categories ?? [];
  const keywords = d.keywords ?? DEFAULT.keywords ?? [];
  return (
    <div className="w-row w3" style={{ marginTop: 44 }}>
      ...
      <div className="widget">
        ...
        <div className="kw-cloud">
          {keywords.map((k, i) => (
            <Link key={k.href ?? i} href={k.href ?? "#"} className={`kw ${k.hot ? "kw-hot" : ""}`}>
